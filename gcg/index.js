...
//配置
let scene,camera,renderer,world,directlight,bg,plane,player,ps,dr
...
function e(){new GLTFLoader().load("light.glb",(sc)=>{scene.add(sc.scene);sc.scene.position.set(0,4,+4);sc.scene.rotation.y=1.573;f()})}
...
var lst2=[[0,2.5,-2],[-2.12255,2.5,-0.4448],[-4.7,2.5,-0.17],[-7.5,2.5,1.78],[-5.2,2.5,2.32],[-8.2,3.5,2.6],[-5.82,3.5,1.17],[-3.25,3.5,1.84],[-2,3.5,0.23],[0,3.5,0]]
var umb2=[]
...
function animate() {
    ...
    player.velocity.x=pv.x
    player.velocity.z=pv.z
    if (type){
    player.applyForce(g)
    }
